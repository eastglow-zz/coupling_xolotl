...
validParams<PETScDMDAMesh>()
{
  ...
  params.addRequiredParam<FileName>("XolotlInput_path_name",
...
PETScDMDAMesh::PETScDMDAMesh(const InputParameters & parameters)
  : MooseMesh(parameters),
    _xolotl_input_path_name(getParam<FileName>("XolotlInput_path_name")),
    _dim(getParam<MooseEnum>("dim"))
{
  ...
  {  
    ...
    interface.initializeXolotl(argc,
              argv, _communicator.get(), false);
    // Now we can get the TS from the app
    TS & ts = xolotl_app->getXolotlTS();
    // Retrieve mesh from TS
    TSGetDM(ts, &_dmda);
  }
  else mooseError("Missing the Xolotl App");
}
