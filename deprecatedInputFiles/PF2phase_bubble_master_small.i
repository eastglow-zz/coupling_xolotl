...
[GlobalParams]
  ...
  var_name_base = etam
  numbub = 2
  bubspac = 150  # Will be ignored if not using PolycrystalVoronoiVoidIC
  radius = 44  # Will be ignored if not using PolycrystalVoronoiVoidIC
  int_width = 30  # Will be ignored if not using PolycrystalVoronoiVoidIC
[]
...
[Functions]
  [./IC_etam0]
    type = ParsedFunction
    value = 'r0:=sqrt((x-${x0})^2+(y-${y0})^2);r1:=sqrt((x-${x1})^2+(y-${y1})^2);if(y >= 601, if(r0<=${rbub},0, if(r1<=${rbub},0,1)) ,0)'
  [../]
  [./IC_etam1]
    type = ParsedFunction
    value = 'r0:=sqrt((x-${x0})^2+(y-${y0})^2);r1:=sqrt((x-${x1})^2+(y-${y1})^2);if(y <= 600, if(r0<=${rbub},0, if(r1<=${rbub},0,1)) ,0)'
  [../]
  [./IC_etab0]
    type = ParsedFunction
...
# Densities
  ...
  [./const]
    ...
    prop_values = '0.5273    0.004688   0.1      2.3398 2.3398 0.0409  0.546    0.454   0.922  1.5 1200  0.2156'
