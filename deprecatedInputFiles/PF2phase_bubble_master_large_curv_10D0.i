...
#XolotlWrapperPath = './xolotl_userobj_12umsq_10dx0.i'
...
#XolotlWrapperPath = '/home/donguk.kim/projects/coupling_xolotl/xolotl_userobj_12umsq_10dx0.i'    #for UF HPG2
XolotlWrapperPath = '/home/donguk.kim/projects/coupling_xolotl/xolotl_userobj_12umsq_10dx0_surfBC.i'    #for UF HPG2
...
[GlobalParams]
  ...
  var_name_base = etam
  numbub = 2
  bubspac = 1500  # Will be ignored if not using PolycrystalVoronoiVoidIC
  radius = 440  # Will be ignored if not using PolycrystalVoronoiVoidIC
  int_width = 300  # Will be ignored if not using PolycrystalVoronoiVoidIC
[]
...
[Functions]
  [./IC_etam0]
    type = ParsedFunction
    value = 'r0:=sqrt((x-${x0})^2+(y-${y0})^2);r1:=sqrt((x-${x1})^2+(y-${y1})^2);if(y >= 6001, if(r0<=${rbub},0, if(r1<=${rbub},0,1)) ,0)'
  [../]
  [./IC_etam1]
    type = ParsedFunction
    value = 'r0:=sqrt((x-${x0})^2+(y-${y0})^2);r1:=sqrt((x-${x1})^2+(y-${y1})^2);if(y <= 6000, if(r0<=${rbub},0, if(r1<=${rbub},0,1)) ,0)'
  [../]
  [./IC_etab0]
    type = ParsedFunction
    value = 'r0:=sqrt((x-${x0})^2+(y-${y0})^2);r1:=sqrt((x-${x1})^2+(y-${y1})^2);if(r0<=${rbub}, 1, if(r1<=${rbub},1,0))'
  [../]

  [./dts]
    type = PiecewiseLinear
    x = '-2000 0   1   10 100  1000 10000 100000 1000000 100000000'
    y = '100   100 0.1 1  10   100  500   1000   5000    5000'
  [../]
[]
...
# Densities
  ...
  [./DmEff]
    type = ParsedMaterial
    f_name = DmEff
    material_property_names = 'Dm'
    args = 'bnds'
    function = 'if(bnds < 0.999, 10*Dm, Dm)'
  [../]
  [./DbEff]
    type = ParsedMaterial
    f_name = DbEff
    material_property_names = 'Db'
    args = 'bnds'
    function = 'if(bnds < 0.999, 10*Db, Db)'
  [../]
  [./cvmatrixeq]
    type = ParsedMaterial
    f_name = cvmatrixeq
  ...
  [./Mobility_v]
    type = DerivativeParsedMaterial
    f_name = Dchiv
    material_property_names = 'DbEff chiv'
    args = 'time'
  ...
  [./Mobility_g]
    type = DerivativeParsedMaterial
    f_name = Dchig
    material_property_names = 'DmEff chig'
    args = 'time'
    #function = 'if(time < 0, 0, DmEff*chig)'
    function = 'DmEff*chig'
    derivative_order = 2
    outputs = exodus
  [../]
